...
public class SelectRelAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<SelectGoodsBean.DataBean> list;
    private Context context;

    public SelectRelAdapter(List<SelectGoodsBean.DataBean> list, Context context) {
        this.list = list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.recitem2, parent, false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        SelectGoodsBean.DataBean dataBean = list.get(position);

        final MyViewHolder vh= (MyViewHolder) holder;
        Uri uri=Uri.parse(dataBean.getImages().split("\\|")[0]);
